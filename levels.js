let quickRegex = {
  ...
  levels: [
    ...
    },
    {
      ...
      description: 'Now let\'s explore the capturing property of <code class="regex">()</code>. Sometimes when you are matching a string, besides the entire match, you may also want to extract subpatterns in the match. For example, besides matching an entire URL, you may also want to extract the protocol, domain name and path of each URL. You can match the entire URL with a regex expression and extract these subpatterns with capturing groups <code class="regex">()</code>.<br><br>More accurately, when there are capturing groups in the regex expression, the regex engine returns what is matched in each <code class="regex">()</code> in addition to the entire match. For example, with the string "apple juice", <code class="regex">(\\w+) (juice)</code> matches the entire string "apple juice" and also returns "apple" and "juice" for the two <code class="regex">()</code> in the expression.<br><br>In this app, capturing groups are marked with <mark class="target-highlight"><mark class="target-group-highlight">dotted lines and a darker shade</mark></mark>. Your regex expression not only has to match the entire pattern, but also has to match the correct capturing groups to pass (by matching the locations of the <mark class="input-highlight"><mark class="input-group-highlight">dotted line markups</mark></mark>).',
